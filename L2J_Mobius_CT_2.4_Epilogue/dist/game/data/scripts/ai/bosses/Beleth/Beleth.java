...
public class Beleth extends AbstractNpcAI
{
	...
	public String onSpellFinished(Npc npc, PlayerInstance player, Skill skill)
	{
		if (!npc.isDead() && !npc.isCastingNow())
		{
			...
			for (PlayerInstance plr : World.getInstance().getVisibleObjectsInRange(npc, PlayerInstance.class, 950))
			{
				npc.setTarget(plr);
				npc.doCast(FIREBALL.getSkill());
				return null;
	...
	{
		npc.setRunning();
		if (!World.getInstance().getVisibleObjectsInRange(npc, PlayerInstance.class, 300).isEmpty() && (getRandom(100) < 60))
		{
			npc.doCast(BLEED.getSkill());
		}
